...
export class DoubaoClient extends BaseLLMClient {
  ...
  async chat(
    ...
    const doubaoMessages = this.parseMessages(messages);

    if (reuseHistory) {
      this.messageHistory = [...this.messageHistory, ...doubaoMessages];
    } else {
      this.messageHistory = doubaoMessages;
    }
    ...
    for (let i = 0; i < modelParameters.maxRetries; i++) {
      try {
        ...
        console.log(
          `Doubao API call failed: ${thisErrorMessage} will sleep for ${sleepTime} seconds and will retry.`
        );
    ...
    if (!response) {
      throw new Error(
        `Failed to get response from Doubao after max retries: ${errorMessage}`
      );
    }
  ...
  private parseMessages(messages: LLMMessage[]): ChatCompletionMessageParam[] {
    const doubaoMessages: ChatCompletionMessageParam[] = [];
